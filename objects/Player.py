...
from objects.TemperatureManager import TemperatureManager
...
class Player(BasicObject):
    ...
    def step(self):
        ...
        self.setTemp()
    def setTemp(self):
        tm = self.findObj(TemperatureManager)
        self.temp = tm.getTempAt(self.rect.left,self.rect.top)
        if abs(self.temp)>25:
            self.hp-=.02
    def buffStep(self):
    ...
    def drawStatus(self,surface):
        ...
        status = "Temp: "+str(self.temp)
        self.drawText(surface,status,(255, 255, 255),0,64)
