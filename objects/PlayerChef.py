...
from objects.FoodDispenser import FoodDispenser
from objects.CounterTop import CounterTop
from objects.Stove import Stove
from objects.Buff import *
from util.Controller import Controller
import numpy
import copy
class PlayerChef(BasicObject):
    ...
    def stepMovement(self):
        ...
        if self.controller.confirm:
            self.leftHand = self.useHand(self.leftHand)
            
        if self.controller.cancel:
            self.rightHand = self.useHand(self.rightHand)
            

        
        self.moveSelf(xMove,yMove)
        
    def getNearestPickup(self):
        targetPickup = None
        nearest = 30
        for a in self.gameObjects:
            if self.distToObj(a) < nearest:
                if isinstance(a, CounterTop):
                    targetPickup = a
                    nearest = self.distToObj(a)
        return targetPickup
        
                    
                
    def useHand(self, hand):
        
        if hand == None:
            hand = self.getNearestPickup().takeFood()
            
        else:
            target = self.getNearestPickup()
            if target.storedFood == None:
                target.storedFood = hand
                hand = None
